...
function DocumentUploadedPage() {
  const { id } = useParams()
  const { document, pages, loading, starting, error, refresh, startConversion } = useDocumentPages(id)
      ...
      <section className="rounded-[2rem] bg-ink p-7 text-white shadow-lift sm:p-9">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">
              {document?.status === 'PREPROCESSING'
                ? 'Every PDF page is now available as an ordered local image. Image preprocessing begins in Phase 4.'
                : 'Convert the PDF into ordered page images before preprocessing and OCR.'}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {(document?.status === 'UPLOADED' || document?.status === 'FAILED') && (
              <button type="button" onClick={startConversion} disabled={starting} className="rounded-full bg-lime px-5 py-2.5 text-sm font-bold text-ink transition hover:bg-white disabled:cursor-wait disabled:opacity-60">
                {starting ? 'Starting…' : document.status === 'FAILED' ? 'Retry conversion' : 'Convert PDF pages'}
              </button>
            )}
        ...
        {document?.status === 'CONVERTING' && (
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-4" aria-live="polite">
            <div className="flex items-center gap-3 text-sm font-semibold"><span className="h-2 w-2 animate-pulse rounded-full bg-lime" />Rendering pages with PyMuPDF…</div>
