...
function PageGallery({ documentId, pages }) {
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        {pages.map((page) => (
          <Link to={`/documents/${documentId}/pages/${page.page_number}`} key={page.id} className="group overflow-hidden rounded-2xl border border-ink/10 bg-white p-2 shadow-sm transition hover:-translate-y-1 hover:shadow-lift">
            ...
            <div className="flex items-center justify-between px-2 pb-1 pt-3">
              ...
              <span className="text-[10px] font-semibold text-ink/40">
                {page.ocr_completed ? `${Math.round(page.average_ocr_confidence * 100)}% OCR` : variant}
              </span>
            </div>
          </Link>
        ))}
