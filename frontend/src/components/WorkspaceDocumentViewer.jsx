...
function WorkspaceDocumentViewer({ documentId, page, loading, variant, onVariantChange, onPrevious, onNext, hasPrevious, hasNext }) {
    ...
    <section className="min-w-0 overflow-hidden rounded-[1.5rem] border border-ink/10 bg-[#e8e5dc] shadow-sm">
      ...
      <div className="relative grid min-h-[32rem] place-items-center p-4 sm:p-6 lg:h-[calc(100vh-22rem)] lg:min-h-[38rem]">
        {loading ? (
          ...
          <img src={getPageImageUrl(imagePath)} alt={`${variant} document page ${page.page_number}`} className="max-h-full max-w-full rounded-sm bg-white object-contain shadow-[0_18px_60px_rgba(19,38,32,0.18)]" />
        ) : (
