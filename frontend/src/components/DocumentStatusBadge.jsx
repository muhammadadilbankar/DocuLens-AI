...
function DocumentStatusBadge({ status }) {
  return (
    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-bold ${statusStyles[status] ?? 'bg-ink/5 text-ink/60'}`}>
      {['CONVERTING', 'PREPROCESSING', 'OCR_PROCESSING', 'EXTRACTING_ENTITIES'].includes(status) && <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-current" />}
